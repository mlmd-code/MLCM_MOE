...
class StackingGating(nn.Module):
    def __init__(self, input_dim, num_experts, num_labels, dropout_rate=0.4, confusion_matrix=None):
        ...
        # 仅使用图像特征的堆叠模型 (输入维度与原始Gating一致)
        self.meta_model = nn.Sequential(
            nn.Linear(input_dim, 128),  # 仅2048维图像特征
            nn.BatchNorm1d(128),
            nn.ReLU(),
    ...
    def forward(self, image_features, expert_labels=None, ml_predictions=None):  # 保持接口兼容
        # 仅使用图像特征
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                all_gating_weights.append(model.gating(image_features, expert_labels, ml_predictions))  # 获取权重
